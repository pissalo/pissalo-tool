 ...
 * Time:10:13:46
 */
class cls_souq_jxc implements jxc_inc
{
    /**
     * @var cls_data
     */
     ...
     * @var string
     */
    private $order_status = '224';
    /**
     * 仓库ID
     * @var string
     */
    private $warehouse_id = '304';
    
    /**
     * cls_souq_jxc constructor.
     */
    ...
    public function cal_first_end_stock($start_month, $end_month)
    {
        ...
        $stock_info = $cls_jxc -> get_jxc_qimo_stock($before_month, 'souq');
        $error_num = 0;
        foreach ($stock_info as $stock) {
            $update_info = array ('sjo_first_stock_num' => $stock['stock']);
            $update_where = "sjo_cangku = 'souq' and sjo_sku = '{$stock['sku']}' and sjo_month = '{$start_month}'";
        ...
        while ($start_month != $end_month) {
            ...
            $now_month_data = $this -> cls_sjo -> select_ex(
                array (
                    'where' => "sjo_month = '{$start_month}' and sjo_cangku = '{$info}'",
            ...
            echo "Souq{$start_month}期初、期末库存处理结束，失败:{$error_num}<br>";
            $count--;
        }
    }
    ...
    public function get_out_in_stock($start_month, $end_month)
    {
        ...
        while ($start_month != $end_month) {
            ...
            $sku_list_now = $cls_jxc -> get_sku_order_chuku_num('souq', $start_month, '/*finance*/osss_num,osss_sku');
            $error_num = 0;
            foreach ($sku_list_now as $sku_info_old) {
                $update_info = array (
                    'sjo_chuku_num' => $sku_info_old['osss_num']
                );
                $update_where = "sjo_sku = '{$sku_info_old['osss_sku']}' and sjo_month = '{$start_month}' and sjo_cangku = 'souq'";
                $flag = $this -> cls_sjo -> update_one($update_info, $update_where);
                $error_num = $flag ? 0 : 1;
            }
            ...
            foreach ($to_list as $to_info) {
                ...
                $update_where = "sjo_sku = '{$to_info['tod_sku']}' and sjo_month = '{$start_month}' and sjo_cangku = 'souq'";
                $flag = $this -> cls_sjo -> update_one($update_info, $update_where);
                $error_num = $flag ? 0 : 1;
            }
            $count--;
            echo "Souq{$start_month}出、入库数据处理结束,失败:{$error_num}<br>";
        }
    }
    ...
    public function get_sku_list($start_month, $end_month)
    {
        ...
        $sku_list_to = $this -> get_instock_sku_list();
        ...
        while ($start_month != $end_month) {
            ...
            $count--;
            echo "Souq{$start_month}SKU生成结束,失败：{$error_num}<br>";
        }
    }
    ...
    public function get_outstock_sku_list()
    {
        ...
        return array_column($cls_osss -> select_ex(
            array (
    ...
    public function get_instock_sku_list()
    {
        $to_sql = "SELECT
            ...
            GROUP BY
                tod_sku";
        return array_column($this -> cls_sjo -> execute($to_sql), 'sku');
    }
    ...
    {
        $this -> cls_sjo -> delete_ex("sjo_cangku='souq'");
    }
}